...
export default function StudentOnlineClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="space-y-6 pb-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            {classData.status !== "completed" && classData.status !== "cancelled" && (
              ...
              >
                ...
                {isLive ? "Join Live Classroom" : "Enter Room"}
              </Link>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* Student Card */}
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">My Details</h3>
            {classData.student ? (
              <div className="flex items-center gap-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 space-y-3">
            <h3 className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-2">
              ...
              Secure Classroom Info
            </h3>
            <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-400">
              <p>• High-definition audio and video streaming</p>
              <p>• Interactive screen sharing and real-time chat</p>
              <p>• Automated attendance tracking on join and exit</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Your join time, exit time, and session duration for this class
              </p>
            </div>
          ...
          {attendance.length === 0 ? (
            <div className="text-center py-16 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-6">
              ...
              <p className="text-xs text-slate-400 max-w-xs mx-auto mt-1">
                Your attendance and duration will be logged automatically when you enter the classroom.
              </p>
            </div>
          ) : (
