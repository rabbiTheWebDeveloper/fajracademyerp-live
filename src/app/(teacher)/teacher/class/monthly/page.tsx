...
function getSessionDate(s: Session) {
...
function CalendarView({
  ...
  const selectedSessions = activeDay ? (byDay[activeDay] || []) : [];
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="grid grid-cols-7">
        {Array.from({ length: totalCells }).map((_, i) => {
          ...
          const dateStr = isValid ? `${year}-${String(mo).padStart(2, "0")}-${String(day).padStart(2, "0")}` : "";
          const isFilterActive = selectedDate === dateStr;
          ...
          return (
            <div
              ...
              onClick={() => {
                if (isValid) {
                  setActiveDay(isDayActive ? null : day);
                  if (dateStr) onSelectDate(isFilterActive ? "" : dateStr);
                }
              }}
              className={`min-h-[72px] sm:min-h-[88px] p-1.5 border-b border-r border-gray-50 transition-all ${
                !isValid ? "bg-gray-50/40" :
                isFilterActive ? "bg-indigo-100/70 ring-2 ring-inset ring-indigo-500" :
                isDayActive ? "bg-indigo-50 ring-2 ring-inset ring-indigo-400" :
              ...
              {isValid && (
                <>
                  <div className={`w-6 h-6 flex items-center justify-center rounded-full mb-1 text-xs font-bold mx-auto ${
                    ...
                    isFilterActive ? "bg-indigo-700 text-white" :
                    isDayActive ? "bg-indigo-200 text-indigo-800" :
                    "text-gray-600"
                  }`}>{day}</div>
                  ...
                  {/* Class count */}
                  {daySessions.length > 0 && (
                    <p className="text-center text-[9px] text-indigo-600 font-semibold mt-0.5">
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-indigo-100">
            <div className="flex items-center gap-2">
              <p className="text-sm font-bold text-gray-800">
                {new Date(year, mo - 1, activeDay).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}
              </p>
              <span className="text-xs font-semibold text-indigo-600 bg-indigo-100 px-2 py-0.5 rounded-full">
                {selectedSessions.length} session{selectedSessions.length !== 1 ? "s" : ""}
              </span>
            </div>
            <button onClick={() => setActiveDay(null)} className="p-1.5 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-white transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="divide-y divide-indigo-100/60 max-h-72 overflow-y-auto">
            {selectedSessions.length === 0 ? (
              <p className="px-5 py-8 text-sm text-center text-gray-400">No classes recorded on this day.</p>
...
export default function MonthlyClassPage() {
  ...
  const fetchData = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      if (calData.success) {
        setAllSessions(calData.sessions || []);
      }
    } catch (e: any) {
      setError(e.message || "Failed to load class sessions");
    } finally {
      setLoading(false);
    }
  }, [month, selectedDate, status, attFilter, search]);
  ...
  // ── Attendance Quick Update Handler ───────────────────────────────────────
  const handleUpdateAttendance = async (sessionId: string, newAtt: "present" | "absent" | "not-marked") => {
    setUpdatingId(sessionId);
    try {
      const res = await fetch("/api/teacher-portal/class-monthly", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          sessionId,
          attendance: newAtt,
          status: newAtt !== "not-marked" ? "completed" : undefined,
        }),
      });
      const data = await res.json();
      if (data.success) {
        setSessions(prev => prev.map(s => s._id === sessionId ? { ...s, studentAttendance: newAtt, status: newAtt !== "not-marked" ? "completed" : s.status } : s));
        setAllSessions(prev => prev.map(s => s._id === sessionId ? { ...s, studentAttendance: newAtt, status: newAtt !== "not-marked" ? "completed" : s.status } : s));
        // Refresh summary stats in background
        fetchData(page);
      }
    } catch (err) {
  ...
  const exportPDF = async () => {
    setExporting(true);
    try {
      const allSessionsList = await fetchAllForPDF();
      const label = selectedDate ? `Date: ${selectedDate}` : monthLabel(month);
      ...
      doc.text("Fajr Academy — Monthly Class & Attendance Report", 14, 11);
      doc.setFontSize(8);
      doc.setFont("helvetica", "normal");
      doc.text(`Period: ${label}   |   Total Sessions: ${summary.total}   |   Generated: ${new Date().toLocaleString()}`, 14, 18);

      // Summary boxes
      const sumData: [string, string][] = [
        ["Total Classes", String(summary.total)],
      ...
      sumData.forEach(([lbl, val], i) => {
        ...
        doc.roundedRect(x, 26, 42, 16, 2, 2, "F");
        doc.setFontSize(7);
      ...
      autoTable(doc, {
        ...
        body: allSessionsList.map((s, i) => [
          i + 1,
          fmtDate(getSessionDate(s)),
        ...
        headStyles: { fillColor: [10, 25, 49], textColor: 255, fontStyle: "bold", fontSize: 7.5 },
        alternateRowStyles: { fillColor: [248, 250, 252] },
        theme: "striped",
      });

      doc.save(`FajrAcademy_ClassReport_${selectedDate || month}.pdf`);
    } catch (e) {
  ...
  return (
    <div className="space-y-4 sm:space-y-5 pb-6 sm:pb-8">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg sm:text-xl font-bold text-gray-900 flex items-center gap-2">
              ...
              Monthly Class & Date-wise Report
            </h1>
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5">
              {selectedDate ? `Date: ${new Date(selectedDate + "T00:00:00").toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}` : monthLabel(month)}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 text-xs sm:text-sm font-medium transition-colors disabled:opacity-50 shadow-sm active:scale-95 cursor-pointer"
          >
          ...
          <button
            ...
            className="ml-auto inline-flex items-center gap-1.5 px-3 sm:px-4 py-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl text-xs sm:text-sm font-semibold shadow-sm hover:shadow-md hover:opacity-90 transition-all disabled:opacity-50 active:scale-95 cursor-pointer"
          >
            {exporting
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-3">
        <StatCard label="Total Classes" value={summary.total} icon={BookOpen} gradient="bg-gradient-to-br from-blue-500 to-indigo-600" loading={loading} />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3.5 sm:p-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 mb-2.5">
            <h3 className="text-sm font-semibold text-gray-700 flex items-center gap-1.5">
              ...
              Attendance Overview {selectedDate ? `(${selectedDate})` : `(${month})`}
            </h3>
      ...
      {/* ── Date-wise Breakdown Pills / Chips ── */}
      {dailyStats.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-bold text-gray-600 uppercase tracking-wider flex items-center gap-1.5">
              ...
              Date-wise Summary ({dailyStats.length} active days in {monthLabel(month)})
            </h3>
            {selectedDate && (
              <button
          ...
          <div className="flex gap-2 overflow-x-auto pb-1.5 scrollbar-thin">
            ...
            >
              All Month ({summary.total})
            </button>
            {dailyStats.map(ds => {
              const isActive = selectedDate === ds._id;
              const dObj = new Date(ds._id + "T00:00:00");
              const dLabel = dObj.toLocaleDateString("en-US", { month: "short", day: "numeric" });
              return (
                <button
                  key={ds._id}
                  onClick={() => setSelectedDate(isActive ? "" : ds._id)}
                  className={`px-3 py-1.5 rounded-xl text-xs whitespace-nowrap transition-all border flex items-center gap-2 cursor-pointer ${
                    isActive
                      ? "bg-indigo-600 text-white border-indigo-600 shadow-sm"
                      : "bg-white hover:bg-indigo-50/50 text-gray-800 border-gray-200"
                  }`}
                >
                  <span className="font-bold">{dLabel}</span>
                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                    isActive ? "bg-white/20 text-white" : "bg-indigo-50 text-indigo-600"
                  }`}>
                    {ds.total} {ds.total === 1 ? "class" : "classes"}
                  </span>
                  <span className={`text-[10px] font-semibold ${isActive ? "text-emerald-200" : "text-emerald-600"}`}>
                    ✓{ds.present}
                  </span>
                </button>
              );
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        >
          <span className="flex items-center gap-2">
            ...
            Filters & Date Range
            {(search || status || attFilter || selectedDate) && (
        ...
        <div className={`p-3 sm:p-4 flex-wrap gap-2.5 sm:gap-3 ${filtersOpen ? "flex" : "hidden sm:flex"}`}>
          {/* Month Selector */}
          <div className="relative">
            ...
            <input
              ...
              type="month"
              value={month}
              onChange={(e) => { setMonth(e.target.value); setSelectedDate(""); setPage(1); }}
              className="pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 bg-white w-full sm:w-auto font-medium"
            />
          </div>
          ...
          <div className="relative flex-1 min-w-[160px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400"
            />
          </div>
          ...
          <div className="relative">
            ...
            <select
              ...
              className="pl-8 pr-7 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 bg-white appearance-none cursor-pointer"
            >
          ...
          {(search || status || attFilter || selectedDate) && (
            <button
              onClick={() => { setSearch(""); setStatus(""); setAttFilter(""); setSelectedDate(""); setPage(1); }}
              className="px-3 py-2 text-xs text-gray-500 hover:text-red-600 border border-gray-200 rounded-xl hover:border-red-200 transition-colors flex items-center gap-1 active:scale-95 cursor-pointer"
            >
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50/60">
            <span className="text-sm font-medium text-gray-700">
              {loading ? "Loading…" : `${pagination.total} session${pagination.total !== 1 ? "s" : ""} ${selectedDate ? `on ${selectedDate}` : `in ${monthLabel(month)}`}`}
            </span>
            ...
            <div className="md:hidden divide-y divide-gray-100">
              {loading ? (
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-1">Try selecting another date or clearing filters.</p>
                </div>
              ) : (
                sessions.map((s) => (
                  <div key={s._id} className="p-4 bg-white hover:bg-gray-50 transition-colors space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        {s.student?.avatar ? (
                      ...
                      <div className="text-right">
                        <p className="text-xs font-bold text-gray-800">{fmtDate(getSessionDate(s))}</p>
                    ...
                    <div className="flex items-center justify-between pt-1">
                      <StatusBadge status={s.status} />
                      <div className="flex items-center gap-1">
                        {(["present", "absent", "not-marked"] as const).map((att) => (
                          <button
                            key={att}
                            disabled={updatingId === s._id}
                            onClick={() => handleUpdateAttendance(s._id, att)}
                            className={`px-2 py-1 rounded text-[10px] font-bold transition-all cursor-pointer ${
                              s.studentAttendance === att
                                ? att === "present" ? "bg-emerald-600 text-white shadow-xs"
                                  : att === "absent" ? "bg-red-600 text-white shadow-xs"
                                  : "bg-gray-600 text-white shadow-xs"
                                : "bg-gray-100 hover:bg-gray-200 text-gray-600"
                            }`}
                          >
                            {att === "present" ? "Present" : att === "absent" ? "Absent" : "Unmark"}
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="text-xs font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100 bg-white">
                    ...
                    <th className="py-3 px-4">Attendance (Quick Update)</th>
                    ...
                    sessions.map((s, i) => (
                      <tr key={s._id} className="border-b border-gray-50 hover:bg-indigo-50/20 transition-colors">
                        ...
                        <td className="py-3 px-4 whitespace-nowrap">
                          <div>
                            <p className="text-xs font-semibold text-gray-800">{fmtDate(getSessionDate(s))}</p>
                            <p className="text-[10px] text-gray-400 capitalize">{s.dayOfWeek}</p>
                          </div>
                        </td>
                        <td className="py-3 px-4">
